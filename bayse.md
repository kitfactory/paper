...
# 答え合わせから見た、確率的機械学習入門

# Step 0.はじめに

この本は機械学習や確率の本を何冊か買っては見たけれど。。。という読者（自分も含め）を対象にしています。確率を使った機械学習を行うには、__「あえて確率的な問題設定を行い，変数の確率分布を求める手順を経る」__ という考え方を獲得するまでの長い旅路になります。

教科書の数式をすべて厳密に理解することも大事ですが、それでは数ページで疲れ果ててしまいます。今から何を目指していくのか、何を知るための準備をしているのか、その大きな流れを知っているのと、いないのでは理解の速さも違ってくると思います。本書のポイントは、教科書の階段が何を達成したいのかを予め理解しておくことです。

この単元では何を抑えようとしているのだろう？それが何につながるのだろう。そのつまづきを支援するために、書きました。さあ、皆さんの教科書を手元にありますか？では、はじめましょう！

# Step1. 確率の説明

まずは確率にもいろいろな種類、書き表し方があることを知りましょう。皆が話す言葉を決めておくことはとても大事ですね。この辺りで理解を目指すのは、確率の書き方、種類があることを理解することです。
...
次に、多くの書籍では確率の代表的な計算方法が説明されるでしょう。それほどパターンが有るわけではありません。以下の３つを覚えておきましょう。

### 加法定理（周辺化）
...
さきほどの節では、ベイズの定理を確率の計算として触れました。単なる\(x\)と\(y\)ということで無味乾燥な式でした。それでは詰まらないですよね？ベイズの定理で狙っていくことは何か、ベイズの定理に出てくる要素をしっかりと覚えておくことが大事です。
...
となります。ここで\(D,w\)の種明かしをすると、\(D\)は観測されたデータ、\(w\)はパラメータがどうなっているのか、つまりは世界(World)と思ってください。

左辺の、\(p(w|D)\)の条件付き確率に注目してみましょう。

__あるデータが\(D\)となった場合、つまりはあるデータ\(D\)を手に入れたとき、\(w\)つまりは世界の状態が、どうなるかを問う式となっています。__ なんと単純で、壮大で、野心的な試みといえる式なのでしょうか。 あなたは世界を予測する式を手に入れたとも言えるのです。

それではベイズの定理の意味が少しわかったところで、ベイズの定理に登場する、世界を予測する式の登場人物を見てみましょう。

まずは左辺の

\(p(w|D)\)　

ですが、 _事後確率__　と呼ばれます。この左辺がデータがわかったときの世界の状態の予測を意味し、ベイズの定理でもっとも重要な項です。では、事後ということは、その前は何でしょう？

それが、右辺の

\(p(w)\)

です。これは __事前確率__ と呼ばれ、ここは世界がどうなっていそうかという仮定でもあります。wのとりうる値は、それぞれどんな確率で起きるかな？、という仮定です。式の流れとしては事前確率から事後確率がわかる。それによって世界を更に良く推定する。それがベイズの定理の力です。

残りの項を見ていきましょう。

* \(p(D|w)\)

こちらは尤度といいます。世界・環境（パラメータ w）があたえられているときにデータが出てくる可能性です。分母の部分はエビデンスともいいます。
...
たとえば、コインの表=1が出る確率を\(λ\)とすると、裏が出る確率は、確率は合計1なので\(1-λ\)と書くことができます。ある一定の確率をもつものが繰り返される、このような確率の分布の仕方を __ベルヌーイ分布__ と呼びます。ベルヌーイ分布の確率は以下のように書くことができます。
