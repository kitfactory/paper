...
=======
となります。ここで\(D,w\)の種明かしをすると、\(D\)は観測されたデータ、\(w\)はパラメータがどうなっているのか、つまりは世界(World)と思ってください。
>>>>>>> d5fdb0712a8f44bd49d070c92f91e632be89d347
...
=======
こちらは __尤度__ といいます。世界・環境（パラメータ w）があたえられているときにデータが出てくる可能性です。最後に分母の部分はエビデンスともいいます。
>>>>>>> d5fdb0712a8f44bd49d070c92f91e632be89d347
...
=======
__エビデンス__ は複雑な式ですね。エビデンスは __周辺尤度__ とも呼ばれます。エビデンスを求めることは計算量が多くなります。ただ、エビデンスは右辺全体を確率1に収める係数となりますので、ある、\(w=1,w=2\)どっちの確率が多いの少ないの？という議論に影響しないということです。計算をしなくてもよい場合があることに注意しましょう。
>>>>>>> d5fdb0712a8f44bd49d070c92f91e632be89d347
...
=======
と書くことができますね？サイコロは1〜6までの離散値を取りますので、このように離散状態について確率を表す関数を __確率質量関数__ といいます。一方でサイコロではなく温度計のような連続的な値を取る場合にも、同様な関数を考えることができますが、その場合、 __確率密度関数__ と呼びます。
>>>>>>> d5fdb0712a8f44bd49d070c92f91e632be89d347
...
そこでλを決める確率分布としてよく使われるのが __ベータ分布__ です。__ベータ分布は0〜1までの値を取る確率変数 \(λ\)に対する確率分布__ です。
...
```plantuml
@startuml
(ベータ分布)->(ベルヌーイ分布): パラメータλを決める
(ベルヌーイ分布)->(0か1か): コインの裏表問題の0か1を決める
@enduml
```
...
学習タスクとしては、次に学習のアプローチに２つあることがわかります。一つは識別モデルです。
...
つまり、__MAP推定は 事後分布で考える__ 方法です。最初に事前分布を仮定して、事後分布を求めます。データが幾つかあるので、それらを使って更新しましょう。
...
ベイズ推定は最尤推定やMAP推定が最大となる点で推定を行ったのに対して __事後分布をそのまま、幅を持ったものとして受け入れ、扱い続けます。__ こんな感じだろう、幅をもったままの状態を扱うということですね。
